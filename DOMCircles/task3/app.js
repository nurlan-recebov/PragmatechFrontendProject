let container = document.querySelector('.container');
let circle = document.createElement('div')

circle.className = 'box'
container.appendChild(circle)
circle.style.width = '400px'
circle.style.height = '400px'
circle.style.background = 'blue'
circle.style.borderRadius = '50%'
circle.style.border = '4px solid white'
circle.style.display = 'flex';
circle.style.justifyContent = 'center'
circle.style.alignItems = 'flex-end'
let circle1 = document.createElement('div')
circle1.className = 'box1'
circle.appendChild(circle1)
circle1.style.width = '300px'
circle1.style.height = '300px'
circle1.style.background = 'blue'
circle1.style.borderRadius = '50%'
circle1.style.border = '4px solid white'
circle1.style.display = 'flex';
circle1.style.justifyContent = 'center'
circle1.style.alignItems = 'flex-end'
let circle2 = document.createElement('div')
circle1.className = 'box1'
circle1.appendChild(circle2)
circle2.style.width = '200px'
circle2.style.height = '200px'
circle2.style.background = 'blue'
circle2.style.borderRadius = '50%'
circle2.style.border = '4px solid white'
circle2.style.display = 'flex';
circle2.style.justifyContent = 'center'
circle2.style.alignItems = 'flex-end'
let circle3 = document.createElement('div')
circle3.className = 'box1'
circle2.appendChild(circle3)
circle3.style.width = '100px'
circle3.style.height = '100px'
circle3.style.background = 'blue'
circle3.style.borderRadius = '50%'
circle3.style.border = '4px solid white'
circle3.style.display = 'flex';
circle3.style.justifyContent = 'center'
circle3.style.alignItems = 'flex-end'